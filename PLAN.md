...
- [x] **Test suite + CI**: `tests/RobloxCS.Tests` (xUnit, net10.0) drives the real pipeline — a harness compiles a C# snippet against `Defs.dll` + BCL ref assemblies (`Basic.Reference.Assemblies`) and runs `Transpiler.Transpile`, asserting the emitted Luau + diagnostics (class→metatable, signed/unsigned int-div, char `++`, `ToString`/`ToString(fmt)`, interpolation format, bitwise→`bit32`, `[Parallel]` wrap, `Parallel.For` fan-out + fallback + thread-safety diagnostic) plus bindgen facts (metatable class, static module, `[WallyPackage]`). 15 facts. `tests/luau/runtime.spec.luau` asserts the platform-independent runtime (`idiv`/`imod` signs, `__pforRanges` coverage, `tostringf`, `hashCode`, `align`, `pfor`, `parallel` serial fallback) via the luau CLI. Both wired into `ci.yml` (`dotnet test` + run the spec + luau-compile the emitted tree). Test project added to `rbxcs.slnx`.

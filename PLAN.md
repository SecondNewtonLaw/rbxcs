...
**Still genuinely external / minor:** publish to nuget.org (needs an account); `Task.Delay` real timing (works on Roblox via `task.delay`, not verifiable off-Roblox); LINQ `First`/`Any` early-break (behaviorally correct, minor perf); `out var` multi-return binding.
